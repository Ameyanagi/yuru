...
対話環境ではデフォルト言語、preview command、preview text extensions、画像 preview protocol、shell binding、shell path backend を尋ねます。`-DefaultLang none`、`-PreviewCommand auto`、`-PreviewImageProtocol none`、`-PathBackend auto`、`-Bindings all` のように指定すると、対話式インストールの既定値を明示できます。
