...
| cmd.exe via Clink | Yuru extension | Requires [Clink](https://chrisant996.github.io/clink/) v1.2.46+ installed (`winget install clink`); `yuru --clink` then prints a Lua script binding `CTRL-T`, `CTRL-R`, and `ALT-C` in cmd.exe. Candidates come from Yuru's own walker, so no `fd` is needed. fzf has no cmd integration. |
