...
```sh
curl -fsSL https://raw.githubusercontent.com/Ameyanagi/yuru/v0.1.11/install | sh -s -- --all --version v0.1.11 --default-lang zh --preview-command auto --preview-image-protocol none --path-backend auto --bindings all
```
...
```powershell
$script = Invoke-RestMethod https://raw.githubusercontent.com/Ameyanagi/yuru/v0.1.11/install.ps1
Invoke-Expression "& { $script } -All -Version v0.1.11 -DefaultLang zh -PreviewCommand auto -PreviewImageProtocol none -PathBackend auto -Bindings all"
```
